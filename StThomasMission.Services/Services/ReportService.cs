    ...
    public class ReportService : IReportService
    {
        ...
        public async Task<byte[]> GenerateStudentReportAsync(int studentId, string format)
        {
            ...
            if (format == "PDF")
            {
                ...
                if (student.Status == "Migrated") document.Add(new Paragraph($"Migrated To: {student.MigratedTo}"));
            ...
            else // Excel
            {
                ...
                worksheet.Cells[5, 2].Value = student.Status;
                if (student.Status == "Migrated")
                {
        ...
        public async Task<byte[]> GenerateClassReportAsync(string grade, int academicYear, string format)
        {
            ...
            else // Excel
            {
                ...
                foreach (var student in students)
                {
                    ...
                    worksheet.Cells[row, 5].Value = student.Status == "Migrated" ? student.MigratedTo : "";
                    row++;
                }
        ...
        public async Task<byte[]> GenerateCatechismReportAsync(int academicYear, string format)
        {
            ...
            else // Excel
            {
                ...
                worksheet.Cells[row + 1, 1].Value = "Trend Analysis (Students by Grade)";
                worksheet.Cells[row + 2, 1].Value = "Grade";
        ...
        public async Task<byte[]> GenerateFamilyReportAsync(string? ward, string? status, string format)
        {
            ...
            if (status != null) families = families.Where(f => f.Status == status);
